...
IW.prototype.associated = function(cb) {
    exec('iwconfig ' + this.iface, function(err, stdout, stderr) {
        ...
        if (status.match(/Not-Associated/)) return cb(true, false)
        cb(false, status)
    })
}

IW.prototype.online = function(cb) {
    dns.lookup('api.unitecontrol.com', function(err, addresses) {
        if (err) return cb(err, false)
        return cb(false, addresses)
    })
}
...
IW.prototype.scan = function(cb) {
    ...
    function parseLine(line) {
        ...
        if (m = /^\s+Quality=(.+?)\//.exec(line)) {
            current.signal = Math.round((m[1] / 70) * 100)
        }
    }
};
